...
test('Update Inventory', async ({ page }) => {
  ...
  await page
    .locator('div')
    .filter({
      hasText: /^Carlos3 Type: EquipmentMedical equipment for surgery$/,
    })
    .locator('[id="_cardMenu_cob55_1"]')
    .click();
  await page.getByRole('button', { name: 'Edit' }).click();
  await page.getByPlaceholder('Inventory Name').click();
  ...
  await expect(
    page.getByText('Inventory updated successfully').first()
  ).toBeVisible();

...
export default function ProductList(): JSX.Element {
  ...
  const [filterType, setFilterType] = useState<string>(''); // State for product type filter

  function FilterByPriceErrorHandling(): boolean {
  ...
  const fetchProducts = async (): Promise<void> => {
    if (!FilterByPriceErrorHandling()) return;
    setIsLoading(true);
    try {
      if (filterType.trim() === '') {
  ...
  const clearFilters = (): void => {
    ...
    setFilterType(''); // Clear the product type filter
  };

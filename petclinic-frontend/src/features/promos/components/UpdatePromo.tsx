...
export default function UpdatePromo(): JSX.Element {
    ...
    useEffect(() => {
        ...
        const fetchPromo = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/v2/gateway/promos/${promoId}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleUpdatePromo = async () => {
        ...
        const formattedExpirationDateString = formattedExpirationDate.toISOString().replace('.999Z', '');
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to update promo: ${response.status} ${response.statusText}`);
            }

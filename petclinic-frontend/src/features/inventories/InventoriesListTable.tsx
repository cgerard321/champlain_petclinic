...
function Reveal({
  ...
  return (
    <div
      style={{
        opacity: ready ? 1 : 0,
        // Important: remove transform once ready to avoid a permanent stacking context
        transform: ready ? 'none' : `translateY(${y}px)`,
        ...
        <div>
          ...
          <div className={cardStylesInventory.cardContainerCustom}>
            {displayedInventories.map((inventory, idx) => {
              ...
              const isMenuOpen = openMenuId === inventory.inventoryId;
              return (
                // New wrapper ensures the whole card (and its menu) stacks above neighbors when open
                <div
                  key={`wrap-${currentPage}-${inventory.inventoryId}`}
                  style={{
                    position: 'relative',
                    zIndex: isMenuOpen ? 2000 : 'auto',
                  }}
                >
                  <Reveal delayMs={idx * 40} y={6}>
                    <div
                      className={`
                      ...
                      style={{
                        ...
                        overflow: isMenuOpen ? 'visible' : undefined,
                      }}
                    >
                      ...
                      <div className={cardStylesInventory.checkboxSection}>
                        ...
                        {openMenuId === inventory.inventoryId && (
                          <div
                            className={cardStylesInventory.popupMenuDiv}
                            style={{ zIndex: 1001 }}
                          >
                            <button
                              onClick={e => {

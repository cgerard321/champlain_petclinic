...
const ProfilePage = (): JSX.Element => {
  ...
  const [isUploadPetPhotoModalOpen, setIsUploadPetPhotoModalOpen] = useState(false);
  const [selectedPetIdForPhoto, setSelectedPetIdForPhoto] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchUserData = async (): Promise<void> => {
      try {
        ...
        if (isMounted) setUserDetails(userDetailsResponse.data);
      } catch {
        if (isMounted) {
          setUserDetails({
            userId: user.userId,
    ...
    const fetchProfilePic = async (): Promise<void> => {
      ...
      try {
        ...
        if (ownerData.photo && ownerData.photo.fileData) {
          ...
          if (isMounted) setProfilePicUrl(objectUrl);
        } else if (isMounted) setProfilePicUrl('');
      } catch {
        if (isMounted) setProfilePicUrl('');
      }
    };
    fetchProfilePic();
    return () => {
  ...
  const fetchPetPhotoUrl = async (
    ...
    try {
      ...
      if (petData.photo && (petData.photo.data || petData.photo.fileData)) {
        ...
        const contentType =
          petData.photo.contentType || petData.photo.fileType || 'image/png';
        const byteCharacters = atob(base64Data);
        const byteNumbers = Array.from(byteCharacters).map(c => c.charCodeAt(0));
        const blob = new Blob([new Uint8Array(byteNumbers)], { type: contentType });
  ...
  useEffect(() => {
    ...
    const fetchOwnerData = async (): Promise<void> => {
      try {
        ...
        const ownerData = ownerResponse.data;
        const petsResponse = await axiosInstance.get(`/owners/${user.userId}/pets`, {
          useV2: false,
        });
        ...
        if (isMounted) setError('Error fetching owner data');
      }
    };
    fetchOwnerData();
    return () => {
  ...
  const handlePetAdded = (newPet: PetResponseModel): void => {
    if (owner) {
      setOwner({ ...owner, pets: [...(owner.pets || []), newPet] });
    }
  };

  const handleOpenUploadPhotoModal = (): void => setIsUploadPhotoModalOpen(true);
  const handleCloseUploadPhotoModal = (): void => setIsUploadPhotoModalOpen(false);

  ...
  const fetchCartItemCount = useCallback(async (): Promise<void> => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/v2/gateway/carts/${cartId}/count`,
        { headers: { Accept: 'application/json' }, credentials: 'include' }
      );
      if (res.ok) {
        const data = await res.json(); // { itemCount: number }
        const count = typeof data?.itemCount === 'number' ? data.itemCount : 0;

...
test.describe('Login Page Tests', () => {
  ...
  test('Form fields are marked as required', async ({ page }) => {
    ...
    await expect(page.locator('#pwd')).toHaveAttribute('required', '');
    await expect(
      page.locator('.help-block:has-text("Email is required")')
    ).toBeAttached();
    await expect(
      page.locator('.help-block:has-text("Password is required")')
    ).toBeAttached();
  });
  ...
  test('Login with valid credentials redirects to welcome page', async ({
    page,
  }) => {
    await page.goto('/#!/login');

...
@Component({
  ...
  template: `
    <style>
      .sortorder:after {content: '\u25b2';   /* BLACK UP-POINTING TRIANGLE */}
      .sortorder.reverse:after {content: '\u25bc';   /* BLACK DOWN-POINTING TRIANGLE */}
    ...
    <div class="button-wrapper">
        ...
        <button class="delete-all-bttn" (click)="deleteAllCancelledVisits()">Delete All Cancelled Visits</button>
    </div>

    <div *ngIf="upcomingVisits.length > 0">
        <h3 style="margin: 0; display: inline-block;">Upcoming Visits</h3>
        <button class="btn btn-link" (click)="showUpcomingVisits = !showUpcomingVisits" style="vertical-align: middle;">
    ...
    <table class="table table-striped" *ngIf="showUpcomingVisits && upcomingVisits.length > 0">
        <thead>
        <tr>
            <th><button class="btn btn-default" (click)="sortBy('visitId')" style="color: white">VisitId<span class="sortorder" *ngIf="propertyName === 'visitId'" [ngClass]="{reverse: reverse}"></span></button></th>
            <th><button class="btn btn-default" (click)="sortBy('visitDate')" style="color: white">Sort by date<span class="sortorder" *ngIf="propertyName === 'visitDate'" [ngClass]="{reverse: reverse}"></span></button></th>
            <th><button class="btn btn-default" (click)="sortBy('description')" style="color: white">Sort by description<span class="sortorder" *ngIf="propertyName === 'description'" [ngClass]="{reverse: reverse}"></span></button></th>
            <th><button class="btn btn-default" (click)="sortBy('practitionerId')" style="color: white">Sort by veterinarian<span class="sortorder" *ngIf="propertyName === 'practitionerId'" [ngClass]="{reverse: reverse}"></span></button></th>
            <th><button class="btn btn-default" (click)="sortBy('petId')" style="color: white">Sort by pet<span class="sortorder" *ngIf="propertyName === 'petId'" [ngClass]="{reverse: reverse}"></span></button></th>
            <th>
        ...
        <tr class="bg-white">
            <td class="border"><input style="width:100%" [(ngModel)]="search.visitId" placeholder="Filter by VisitID"></td>
            <td class="border"><input style="width:100%" [(ngModel)]="search.date" placeholder="Filter by Date"></td>
            <td class="border"><input style="width:100%" [(ngModel)]="search.description" placeholder="Filter by Description"></td>
            <td class="border"><input style="width:100%" [(ngModel)]="search.vetId" placeholder="Filter by Veterinarian"></td>
            <td class="border"><input style="width:100%" [(ngModel)]="search.petId" placeholder="Filter by PetId"></td>
        ...
        <tr id="upcomingVisitId" *ngFor="let v of upcomingVisits | orderBy:propertyName:reverse | filter:search:query; trackBy: trackByVisitId" data-table-name="upcomingVisits">
            <td class="border"><a [routerLink]="['/visits', v.visitId]"> {{ v.visitId }}</a></td>
            ...
            <td class="border">
                <a [ngClass]="{
                    ...
                    }" href="javascript:void(0)" (click)="confirmVisit(v.visitId, v.status)">
            ...
            <td class="border"><a [ngClass]="{
        'btn btn-cancel-upcoming': v.status === 'UPCOMING',
        'btn btn-cancel-disabled': v.status === 'CANCELLED' || v.status === 'CONFIRMED' || v.status === 'COMPLETED'
      }" href="javascript:void(0)" (click)="cancelVisit(v.visitId, v.status)">
            ...
            <td class="border" style="text-align:center; vertical-align:middle"><button class="btn btn-edit-royal" type="button" (click)="switchToUpdateForm($event, v.practitionerId, v.description, v.visitId, v.status)">Edit Visit</button></td>
            <td class="border"><a class="btn btn-delete-red" href="javascript:void(0)" (click)="deleteVisit(v.visitId)">Delete Visit</a></td>
        </tr>
        </tbody>
    </table>
    ...
    <div *ngIf="confirmedVisits.length > 0">
        ...
        <button class="btn btn-link" (click)="showConfirmedVisits = !showConfirmedVisits" style="vertical-align: middle;">
    ...
    <table class="table table-striped" *ngIf="showConfirmedVisits && confirmedVisits.length > 0">
        ...
        <tr id="confirmedVisitId" *ngFor="let v of confirmedVisits | orderBy:propertyName:reverse | filter:search:query; trackBy: trackByVisitId" data-table-name="confirmedVisits">
            <td class="border"><a [routerLink]="['/visits', v.visitId]"> {{ v.visitId }}</a></td>
            <td class="border">{{v.visitDate | date:'yyyy-MM-ddTHH:mm:ss'}}</td>
            <td class="border" style="white-space: pre-line">{{v.description}}</td>
            <td class="border" style="white-space: pre-line">{{v.vetFirstName}} {{v.vetLastName}}</td>
    ...
    <div *ngIf="cancelledVisits.length > 0">
        ...
        <button class="btn btn-link" (click)="showCancelledVisits = !showCancelledVisits" style="vertical-align: middle;">
    ...
    <table class="table table-striped" *ngIf="showCancelledVisits && cancelledVisits.length > 0">
        ...
        <tr id="cancelledVisitId" *ngFor="let v of cancelledVisits | orderBy:propertyName:reverse | filter:search:query; trackBy: trackByVisitId" data-table-name="cancelledVisits">
            <td class="border"><a [routerLink]="['/visits', v.visitId]"> {{ v.visitId }}</a></td>
            <td class="border">{{v.visitDate | date:'yyyy-MM-ddTHH:mm:ss'}}</td>
            <td class="border" style="white-space: pre-line">{{v.description}}</td>
            <td class="border" style="white-space: pre-line">{{v.vetFirstName}} {{v.vetLastName}}</td>
    ...
    <div *ngIf="completedVisits.length > 0">
        ...
        <button class="btn btn-link" (click)="showCompletedVisits = !showCompletedVisits" style="vertical-align: middle;">
    ...
    <table class="table table-striped" *ngIf="showCompletedVisits && completedVisits.length > 0">
        ...
        <tr id="completedVisitId" *ngFor="let v of completedVisits | orderBy:propertyName:reverse | filter:search:query; trackBy: trackByVisitId" data-table-name="completedVisits">
            <td class="border"><a [routerLink]="['/visits', v.visitId]"> {{ v.visitId }}</a></td>
            <td class="border">{{v.visitDate | date:'yyyy-MM-ddTHH:mm:ss'}}</td>
            <td class="border" style="white-space: pre-line">{{v.description}}</td>
            <td class="border" style="white-space: pre-line">{{v.vetFirstName}} {{v.vetLastName}}</td>
...
export class VisitListComponent implements OnInit, OnDestroy {
  ...
  confirmVisit(visitId: string, status: string): void {
    ...
    this.visitApi.updateVisitStatus(visitId, newStatus as any).subscribe({
      next: () => {
  ...
  cancelVisit(visitId: string, status: string): void {
    ...
    this.visitApi.updateVisitStatus(visitId, 'CANCELLED' as any).subscribe({
      next: () => {
  ...
  deleteVisit(visitId: string): void {
    const confirmed = confirm('You are about to delete visit ' + visitId + '. Is this what you want to do?');
    if (confirmed) {
  ...
  deleteAllCancelledVisits(): void {
    const confirmed = confirm('You are about to delete all canceled visits. Is this what you want to do?');
    if (confirmed) {
  ...
  switchToUpdateForm(_event: Event, _practitionerId: string, _description: string, _visitId: string, _status: string): void {

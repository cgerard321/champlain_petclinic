...
export class InventoryApiService {
  ...
  getAllInventories(
    ...
    if (searchParams) {
      ...
      if (searchParams.inventoryDescription)
        params.push(`inventoryDescription=${searchParams.inventoryDescription}`);
    }
  ...
  createInventoryType(typeName: { name: string }): Observable<unknown> {
    return this.http.post<unknown>(`${this.apiConfig.getFullUrl('/inventories')}/types`, typeName);
  }
  ...
  updateInventory(inventoryId: string, inventory: InventoryRequest): Observable<Inventory> {
    return this.http.put<Inventory>(
      `${this.apiConfig.getFullUrl('/inventories')}/${inventoryId}`,
      inventory
    );
  }
  ...
  getProductById(productId: string): Observable<InventoryProduct> {
    return this.http.get<InventoryProduct>(
      `${this.apiConfig.getFullUrl('/inventories')}/products/${productId}`
    );
  }

  createProduct(product: ProductRequest): Observable<InventoryProduct> {
    return this.http.post<InventoryProduct>(
      `${this.apiConfig.getFullUrl('/inventories')}/${product.inventoryId}/products`,
      product
    );
  }
  ...
  updateProduct(productId: string, product: ProductRequest): Observable<InventoryProduct> {
    return this.http.put<InventoryProduct>(
      `${this.apiConfig.getFullUrl('/inventories')}/${product.inventoryId}/products/${productId}`,
      product
    );
  }

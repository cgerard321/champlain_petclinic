    ...
    <div class="container mt-5" *ngIf="bills">
      <div class="row">
        ...
        <div class="col-md-6 text-end">
          <a class="newBtn btn btn-primary" [routerLink]="['/bills', bills.billId, 'edit']">Update Bill</a>
        </div>
      </div>
      <br>
      ...
      <div class="row g-5">
        <div class="col-md-6">
          ...
          <hr>
          <br>
          <h5>Bill Status: <span [ngStyle]="{'color': bills.billStatus === 'OVERDUE' ? 'red' : 'black'}">{{bills.billStatus}}</span></h5>
          <hr>
          <br>
          <h5>Due Date: <strong>{{bills.dueDate}}</strong></h5>
          <hr>
          <br>
          <h5>Final Amount Due: {{bills.amount}}</h5>
          <h5 *ngIf="show">Final Amount Including Taxes: {{bills.taxedAmount}}</h5>
          <button class="newBtn btn btn-primary" (click)="show = !show"> {{show? "Hide amount including taxes" : "Show amount including taxes"}}</button>
          <hr>
          <br>
          <div>

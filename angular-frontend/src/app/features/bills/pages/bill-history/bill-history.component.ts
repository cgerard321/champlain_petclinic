      ...
      <div class="btn-toolbar mb-3 toolbar-gap" role="toolbar" aria-label="Bills toolbar">
        <div class="btn-group" role="group">
          <button
            ...
            [class.active]="toolbar.open === 'search'"
        ...
        <div class="btn-group" role="group">
          <button
            class="btn btn-primary"
            [class.active]="toolbar.open === 'filter'"
        ...
        <div class="btn-group" role="group">
          <button class="btn btn-outline-danger" (click)="deleteAllBills()">
            Delete All Bills
          </button>
        </div>
      </div>
          ...
          <tr *ngFor="let bill of getRows(); trackBy: trackByBillId">
            ...
            <td>
              <a
                ...
                [routerLink]="['/owners', getOwnerUUIDByCustomerId(bill.customerId)]"
              >
                View Owner
              </a>
            </td>
            ...
            <td>
              <span
                ...
                [ngClass]="{
                  'bg-success':
                    activeStatus == 'paid' || (bill.status || '').toLowerCase() == 'paid',
                  'bg-secondary':
                    activeStatus == 'unpaid' || (bill.status || '').toLowerCase() == 'unpaid',
                  'bg-danger':
                    activeStatus == 'overdue' || (bill.status || '').toLowerCase() == 'overdue',
                }"
              >
            ...
            <td>
              <a
                ...
                [routerLink]="['/bills', bill.billId || bill.id]"
                >Details</a
              >
            </td>
            <td>
              <button
                ...
                (click)="deleteBill(bill.billId || bill.id)"
              >
                Delete
              </button>
            </td>
...
export class BillHistoryComponent implements OnInit {
  ...
  getCustomerDetails(customerId: string): string {
    return (
      this.customerNameMap[customerId] || this.customerNameMap2[customerId] || 'Unknown Customer'
    );
  }
  ...
  deleteBill(billId: string): void {
    const ok = confirm(
      'You are about to delete billId ' + billId + '. Is it what you want to do ? '
    );
    if (!ok) return;

...
angular
  .module('inventoriesUpdateForm')
  .controller('InventoriesUpdateFormController', [
    '$http',
    '$state',
    '$stateParams',
    '$scope',
    function ($http, $state, $stateParams, $scope) {
      ...
      $http.get('/api/gateway/inventories/' + inventoryId).then(function (
        resp
      ) {
        ...
        $http.get('/api/gateway/inventories/types').then(function (typesResp) {
          ...
          if ($scope.inventoryTypeUpdateOptions.includes(inventoryType)) {
            ...
            $scope.inventoryTypeFormUpdateSearch =
              $scope.inventoryTypeUpdateOptions[0];
          }
        }, handleHttpError);
      }, handleHttpError);
      ...
      self.submitUpdateInventoryForm = function () {
        ...
        if (
          ...
          $http
            .post('/api/gateway/inventories/types', {
              type: $scope.selectedUpdateOption,
            })
            .then(function () {
              if (method === 'edit') {
                $http
                  .put('/api/gateway/inventories/' + inventoryId, data)
                  .then(function () {

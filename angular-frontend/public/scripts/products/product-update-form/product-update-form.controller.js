...
angular.module('productUpdateForm').controller('ProductUpdateFormController', [
  '$http',
  '$state',
  '$stateParams',
  function ($http, $state, $stateParams) {
    ...
    $http.get('/api/gateway/products/' + productId).then(function (resp) {
      ...
      if (
        self.product.isUnlisted === undefined ||
        self.product.isUnlisted === null
      ) {
      ...
      //fetch image
      $http
        .get('api/gateway/images/' + prod.imageId)
    ...
    self.submitProductUpdateForm = function () {
      ...
      $http.put('/api/gateway/products/' + productId, data).then(
        function () {

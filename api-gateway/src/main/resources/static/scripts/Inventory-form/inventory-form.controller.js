...
angular.module("inventoryForm").controller("InventoryFormController", [
  "$http",
  "$state",
  "$stateParams",
  "$scope",
  function ($http, $state, $stateParams, $scope) {
    ...
    self.submitInventoryForm = function () {
      ...
      if (
        ...
        $http
          .post("api/gateway/inventory/type", { type: $scope.selectedOption })
          .then(function (resp) {
            $http.post("api/gateway/inventory", data).then(
              function (resp) {
                console.log(resp);

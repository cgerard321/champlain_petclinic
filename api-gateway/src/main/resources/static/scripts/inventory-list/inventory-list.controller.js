...
angular.module("inventoryList").controller("InventoryListController", [
  "$http",
  "$scope",
  "$stateParams",
  "$state",
  function ($http, $scope, $stateParams, $state) {
    ...
    $scope.deleteAllInventories = function () {
      ...
      if (varIsConf) {
        $http.delete("api/gateway/inventory").then(
          function (response) {
    ...
    function proceedToDelete(inventory) {
      ...
      $http
        .delete("api/gateway/inventory/" + inventory.inventoryId)
      ...
      function successCallback(response) {
        ...
        setTimeout(() => {
          showNotification(
            inventory.inventoryName + " has been deleted successfully!",
          );

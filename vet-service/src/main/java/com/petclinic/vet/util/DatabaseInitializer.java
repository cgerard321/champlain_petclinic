    ...
    private Mono<Void> initializeDatabase(ConnectionFactory connectionFactory) {
        return Mono.from(connectionFactory.create())
                .flatMap(connection -> {
                    ...
                    String createBadgesTableSql = "CREATE TABLE IF NOT EXISTS badges (id INT PRIMARY KEY, vet_id VARCHAR(255) UNIQUE, badge_title VARCHAR(255), badge_date VARCHAR(255), img_data bytea)";

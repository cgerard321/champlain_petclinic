...
angular.module('inventoriesList').controller('InventoriesListController', [
  '$http',
  '$scope',
  '$stateParams',
  '$state',
  function ($http, $scope, $stateParams, $state) {
    ...
    $http.get('api/gateway/inventories').then(function (resp) {
      ...
      // console.log removed('Resp data: ' + resp.data);
      // console.log removed('inventory list: ' + self.inventoryList);
    });
    ...
    function getInventoryList(
      ...
      $state.transitionTo(
        'inventories',
        { page: self.currentPage, size: self.listSize },
        { notify: false }
      );
      var queryString = '';
      name = '';
    ...
    $scope.deleteAllInventories = function () {
      let varIsConf = confirm(
        'Are you sure you want to clear all entries from the inventory?'
      );
      if (varIsConf) {
        $http.delete('api/gateway/inventories').then(
          function () {
            alert('All inventory entries have been cleared!');

            $http.get('api/gateway/inventories').then(function () {
              self.inventoryList = [];
            });
          },
          function (error) {
            alert(error.data.errors);
            // console.log removed(error, 'Failed to clear inventory entries.');
          }
        );
      }
    };
    ...
    function proceedToDelete(inventory) {
      ...
      $http
        .delete('api/gateway/inventories/' + inventory.inventoryId)

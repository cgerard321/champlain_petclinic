...
angular.module('inventoriesForm').controller('InventoriesFormController', [
  '$http',
  '$state',
  '$stateParams',
  '$scope',
  function ($http, $state, $stateParams, $scope) {
    var self = this;
    // console.log removed('State params: ' + $stateParams);
    ...
    self.submitInventoryForm = function () {
      ...
      if (
        ...
        $http.post('api/gateway/inventories', data).then(function () {
          // console.log removed(resp);
          $state.go('inventories');
        }, handleHttpError);
      }

...
angular.module('billHistory').controller('BillHistoryController', [
  ...
  function ($http, $stateParams, $scope) {
    ...
    vm.getRows = function () {
      switch (vm.activeStatus) {
        case 'paid':
          return vm.paidBills;
        case 'unpaid':
          return vm.unpaidBills;
        case 'overdue':
          return vm.overdueBills;
        default:
          return vm.billHistory;
      }
    };
    ...
    vm.visitTypePredicate = function (bill) {
      ...
      var v =
        bill && bill.visitType ? String(bill.visitType).toLowerCase() : '';
    ...
    function loadDefaultData() {
      if (!vm.searchActive) {
        loadTotalItemForDefaultData().then(function (totalItems) {
          ...
          $http
            .get(
              vm.baseURL + '?page=' + vm.currentPage + '&size=' + vm.pageSize
            )
            .then(function (resp) {
    ...
    eventSource.onerror = function () {
      if (eventSource.readyState === 0) eventSource.close();
    };
    ...
    eventSourcePaid.onerror = function () {
      if (eventSourcePaid.readyState === 0) eventSourcePaid.close();
    };
    ...
    eventSourceUnpaid.onerror = function () {
      if (eventSourceUnpaid.readyState === 0) eventSourceUnpaid.close();
    };
    ...
    eventSourceOverdue.onerror = function () {
      if (eventSourceOverdue.readyState === 0) eventSourceOverdue.close();
    };
    ...
    $scope.getOwnerUUIDByCustomerId = function (customerId) {
      let foundOwner;
      vm.owners.forEach(function (o) {
        if (o.ownerId === String(customerId)) foundOwner = o;
      });
      vm.ownersUUID.forEach(function (o) {
        if (o.ownerId === String(customerId)) foundOwner = o;
      });
      if (foundOwner && vm.ownersInfoArray) {
        const match = vm.ownersInfoArray.find(function (o) {
          return (
            o.firstName === foundOwner.firstName &&
            o.lastName === foundOwner.lastName
          );
        });
    ...
    $scope.getVetDetails = function (vetId) {
      ...
      const viaBillId = vm.vetList.find(function (v) {
        return v.vetBillId === vetId;
      });
      const viaVetId = vm.vetList.find(function (v) {
        return v.vetId === vetId;
      });
    ...
    vm.customerNameMap = {};
    vm.customerNameMap2 = {};
    vm.owners.forEach(function (c) {
      vm.customerNameMap[c.ownerId] = c.firstName + ' ' + c.lastName;
    });
    vm.ownersUUID.forEach(function (c) {
      vm.customerNameMap2[c.ownerId] = c.firstName + ' ' + c.lastName;
    });

    $scope.getCustomerDetails = function (customerId) {
      return (
        vm.customerNameMap[customerId] ||
        vm.customerNameMap2[customerId] ||
        'Unknown Customer'
      );
    };

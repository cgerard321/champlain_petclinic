  ...
  function ($http, $state, $stateParams) {
    ...
    if (!vetId || vetId === 0) {
      ...
      $http.get('api/gateway/vets/' + $stateParams.vetId).then(function (resp) {
        self.vet = resp.data;
        // TODO: Fix linting error - // TODO: Fix linting error - console.log removed for linting compliance
        // console.log removed for linting compliance
        // // TODO: Fix linting error - console.log removed for linting compliance
    ...
    let uploadPhoto = function (vetId) {
      ...
      var image = {};
      reader.onloadend = () => {
        vetPhoto = reader.result.replace('data:', '').replace(/^.+,/, '');
        ...
        $http
          .post(`api/gateway/vets/${vetId}/photos/${image.name}`, image) // Send the image object
          .then(function () {
            // TODO: Fix linting error - unused 'response' parameter removed
            // TODO: Fix linting error - console.log removed for linting compliance
    ...
    let updatePhoto = function (vetId) {
      ...
      reader.onloadend = () => {
        ...
        // Use template literals for URL concatenation
        $http
          .put(`api/gateway/vets/${vetId}/photos/${image.name}`, image) // Send the image object
          .then(function () {
            // TODO: Fix linting error - unused 'response' parameter removed
            // TODO: Fix linting error - console.log removed for linting compliance
            // console.log('VET ID: ' + vetId);
    ...
    self.submitVetForm = function (vet = self.vet) {
      ...
      for (let i = 0; i < selectedSpecialtiesList.length; i++) {
        if (selectedSpecialtiesList[i].checked) {
          ...
          if (i !== selectedSpecialtiesList.length - 1) specialtiesStr += ', ';
        }
      }
      specialtiesStr += ']';
      if (specialtiesStr == '[]') {

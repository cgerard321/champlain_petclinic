...
const petClinicApp = angular.module('petClinicApp', [
  ...
  'inventoriesProductUpdateForm',
  'inventoriesProductDetailsInfo',
  'inventoriesProductForm',
  'verification',
  'adminPanel',
  'resetPwdForm',
  'forgotPwdForm',
...
petClinicApp.factory('authProvider', [
  '$window',
  function ($window) {
    return {
...
petClinicApp.factory('httpErrorInterceptor', [
  '$q',
  '$location',
  'authProvider',
  function ($q, $location, authProvider) {
    return {
      responseError: rej => {
        if (
          !whiteList.has($location.path().substring(1)) &&
          rej.status === 401
        ) {
...
petClinicApp.run([
  ...
  function ($rootScope, $location, authProvider) {
    $rootScope.$on('$locationChangeSuccess', function (event) {
      if (
        whiteList.has($location.path().substring(1)) ||
        $location.path().startsWith('/reset_password')
      ) {
        return;
      }
...
['welcome', 'nav', 'footer'].forEach(function (c) {
  ...
  angular.module(mod, []);
  angular.module(mod).controller(controller, [
    '$rootScope',
    '$scope',
    'authProvider',
    function ($rootScope, $scope, authProvider) {
      const load = () => {
